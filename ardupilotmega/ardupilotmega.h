...
typedef enum MAV_CMD
{
   ...
   MAV_CMD_VIDEO_START_CAPTURE=2500, /* Starts video capture |Camera ID (0 for all cameras), 1 for first, 2 for second, etc.| Frames per second| Resolution in megapixels (0.3 for 640x480, 1.3 for 1280x720, etc), set to 0 if param 4/5 are used| WIP: Resolution horizontal in pixels| WIP: Resolution vertical in pixels| WIP: Bit rate (0 for auto)| WIP: Video image rotation clockwise (0-359 degrees)|  */
   MAV_CMD_VIDEO_STOP_CAPTURE=2501, /* Stop the current video capture |WIP: Camera ID| Reserved|  */
   MAV_CMD_REQUEST_VIDEO_STREAM_TARGET=2502, /* WIP: Request video stream target (VIDEO_STREAM_TARGET) |1: Request video stream target| Camera ID| Reserved (all remaining params)|  */
